...
cost the tier curve a claim. The most valuable single report available now is
a 16 GB Mac with a *fast* SSD: the two measured rows differ in both memory and
disk speed, so nothing yet separates the two axes. After that, an older chip,
a fanless Air, or an external SSD, since those are exactly the conditions the
48 GB curve cannot see.
