...
| `--mirror <dir>` | A directory holding a byte-identical copy of `--model` on another disk. Repeatable. Every checkpoint-shard read goes to whichever copy is estimated to answer first, so the copies need not be equally fast; see [Mirrored checkpoints](#mirrored-checkpoints). |
...
(200 greedy tokens, one prompt, MTP on, 118 experts per layer), on a Mac mini M4 whose external NVMe reads 3.18 GB/s and
whose internal SSD reads 1.81 GB/s, a mirror raises prefill from 3.2 to
...
corruption with unchanged headers will not be detected. Verify the copies byte
for byte before use. The historical throughput figures above have not been
re-measured on the rebased main branch.

For example, after copying and verifying the complete checkpoint:
