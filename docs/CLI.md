...
The copies do not have to be equally fast, because nothing about the devices is
configured: each replica's throughput is learned from its own completed reads,
and the split follows. On a Mac mini M4 whose external NVMe reads 3.18 GB/s and
...
that is not the same checkpoint is refused rather than silently mixed in.
