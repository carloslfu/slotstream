...
At 22.57% of 6,811 library lines, these are the honest gaps and their reasons:

| File | Why it is uncovered |
|---|---|
| `Layers.swift`, `Model.swift`, `MTP.swift`, `Weights.swift`, `Vendored/GatedDelta.swift` | The model itself. Needs a checkpoint; covered on the dev Mac by parity and the equality gates. A synthetic checkpoint would bring these to CI. |
| `Server.swift` (94%) | The socket loop and the request handlers. The framing and routing rules are split out and covered; the handlers still need an engine. |
| `WeightDownload.swift` | The download engine. Needs a server to talk to; a local Range server would cover resume and fallback, which have no gate today. |
| `Engine.swift`, `ExpertStore.swift`, `NgramStore.swift`, `Checkpoint.swift` | Load and generation paths. Weights. |

None of this is untested — `verify.sh` and `api_robustness.sh` exercise most of
it against the real model. It is untested *on CI*, which is a different and
weaker claim, and the table says which is which.
