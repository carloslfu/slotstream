...
| A head already written at that boundary is upgraded, never short-circuited (`PersistentPrefixSave`, the existing-head check) | Correctness bound | The engine writes a conversation's own checkpoint at its last resume boundary, and a shared prefix is saved at the last pass end at or before the prompt's system boundary, so the two land on one position whenever the system message ends inside the same pass cell as that boundary; the checkpoint write comes first and without the flag. Answering the shared save as already present left the head classed as the conversation's, and a later save removed it as a redundant ancestor, so no other conversation could start from it. The upgrade rewrites that one head (115.9 MB per state in the live directory) and references its rows instead of writing them again. | None. Revise if the extra head write measurably costs more than the reuse it enables; a shared save that is skipped or fails is reported to the server's prefix-cache log instead of only being counted. |
