---
type: run
created: 2026-09-04T01:45:00-05:00
updated: 2026-09-04T01:45:00-05:00
captured_at: 2026-09-04T01:45:00-05:00
summary: 'verify.sh with the vision gates on an idle machine: 24 passed, 0 failed, including vision parity and 18 serving assertions at the 8.1 GB floor.'
...
title: verify.sh with the vision gates, idle dev Mac, 2026-09-04
note: 'The clean re-run of the earlier battery, with 37.6 GB reclaimable and no other model process. Supersedes nothing: the earlier run''s one failure was its server being killed by the kernel while an unrelated `slotstream serve` held the machine, and both are kept.'
---

The full acceptance battery, plus the standalone serving suite at the floor
that the gate itself runs.
...
PASS  and it recognises the dog
      -> 'Dog nose close-up' in 15.9s, 725 prompt tokens
...
## The serving suite, standalone, at --memory-gb 8.1

```
PASS  ollama /api/chat answers an image request
PASS  and it recognises the dog
      -> 'Dog nose close-up.' in 14.7s, 725 prompt tokens
PASS  the picture is worth its 702 placeholder tokens, plus the two sentinels
PASS  /v1/chat/completions answers an image_url part
PASS  and it sees the fruit on the tree
      -> 'Green citrus fruit'
