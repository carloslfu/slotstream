...
**Gates.** The check catalogue passes on the committed build: 46 checks, 0 failed, 0 skipped, 28,499 assertions. The routing rule is exercised by the `checkpoint-mirror` T0 check in `Diagnostics+CheckpointMirror.swift`, which covers single-copy routing, failed-read accounting, unmeasured-replica probing, backlog spillover, stale-measurement reprobe and concurrent claim/release across three replicas, all against an injectable clock with no weights or disk I/O. End-to-end behaviour across two real disks is covered by the A/B and the two witnesses above; it cannot live in the catalogue because it needs a second copy of a real model on a second disk.
...
**Gates.** None of these concurrency-knob measurements are reproduced by the build. The routing rule itself is exercised by the `checkpoint-mirror` T0 check, but the queue-depth and prefetch-lane interactions measured here are end-to-end behaviours that depend on a real model and two disks.
